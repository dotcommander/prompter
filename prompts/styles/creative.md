...
2. **Open the space** — add language that encourages exploration and multiple perspectives
3. **Sensory details** — prompt for vivid, concrete descriptions when relevant
4. **Divergent thinking** — ask for unexpected connections or unconventional approaches
5. **Tone and voice** — specify the emotional register or narrative style if applicable
6. **Structure freedom** — allow flexible output formats unless the user needs something specific
7. **Follow-up hooks** — include prompts for depth ("elaborate on...", "what if...")

## Output

Return ONLY the enhanced prompt. No commentary. Output the raw prompt text directly.
